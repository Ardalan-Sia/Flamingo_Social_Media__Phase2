package ap.mini_project.client.graphic.panels;

import ap.mini_project.client.listener.EventListener;
import ap.mini_project.client.listener.StringListener;
import ap.mini_project.shared.events.SignUpEvent;

import javax.swing.*;
import java.awt.*;

public class RegistrationPanel extends AbstractPanel {
    JButton registrationBtn;
    JLabel message;
    JButton backBtn;
    JTextField usernameField;
    JTextField passField;
    JTextField passCheckField;
    StringListener stringListener;

    private final EventListener eventListener;

    public RegistrationPanel(EventListener eventListener) {
        setLayout(new GridBagLayout());

        GridBagConstraints gc = new GridBagConstraints();

        this.eventListener = eventListener;


        usernameField = new JTextField(10);

        passField = new JTextField(10);

        passCheckField = new JTextField(10);

        registrationBtn = new JButton("register");
        registrationBtn.addActionListener(e -> registerAction());

        backBtn = new JButton("back");
        backBtn.addActionListener(e-> back());

        initializeMessage();

        ////////////////1
        gc.gridx = 0;
        gc.gridy = 0;

        gc.insets = new Insets(0,0,0,0);
        gc.anchor = GridBagConstraints.LINE_END;
        add(new JLabel("username : ") , gc);

        gc.gridx = 1;
        gc.gridy = 0;

        gc.insets = new Insets(0,0,0,0);
        gc.anchor = GridBagConstraints.LINE_START;
        add(usernameField , gc);

        /////////////////2
        gc.gridx = 0;
        gc.gridy = 1;

        gc.insets = new Insets(0,0,0,0);
        gc.anchor = GridBagConstraints.LINE_END;
        add(new JLabel("password : ") , gc);

        gc.gridx = 1;
        gc.gridy = 1;

        gc.insets = new Insets(0,0,0,0);
        gc.anchor = GridBagConstraints.LINE_START;
        add(passField , gc);

        ///////////////3

        gc.gridx = 0;
        gc.gridy = 2;

        gc.insets = new Insets(0,0,0,0);
        gc.anchor = GridBagConstraints.LINE_END;
        add(new JLabel("password : ") , gc);

        gc.gridx = 1;
        gc.gridy = 2;

        gc.insets = new Insets(0,0,0,0);
        gc.anchor = GridBagConstraints.LINE_START;
        add(passCheckField , gc);

        ////////////////4
        gc.gridx = 1;
        gc.gridy = 3;

        gc.insets = new Insets(5,0,0,0);
        gc.anchor = GridBagConstraints.CENTER;

        add(message, gc);

        ////////////////5

        gc.gridx = 0;
        gc.gridy = 4;

        gc.insets = new Insets(5,0,0,0);
        gc.anchor = GridBagConstraints.CENTER;

        add(registrationBtn, gc);

        gc.gridx = 1;
        gc.gridy = 4;

        gc.insets = new Insets(0,0,0,0);
        gc.anchor = GridBagConstraints.LINE_END;

        add(backBtn,gc);



    }

    private void back(){
        stringListener.stringEventOccurred("backToAuth");
    }
    public void setStringListener(StringListener stringListener){
        this.stringListener = stringListener;

    }
    public void setMessage(String message){
        this.message.setText(message);
    }


    private void initializeMessage() {
        message = new JLabel("", SwingConstants.CENTER);
        message.setForeground(Color.RED);
    }

    private void registerAction(){
        eventListener.listen(new SignUpEvent(usernameField.getText() , passField.getText()));
    }
}
